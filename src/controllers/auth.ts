...
type RestaurantType = {
  ...
  address: string;
  address_link: string;
  start_date: string; // Opening time (e.g., "09:00")
  end_date: string; // Closing time (e.g., "22:00")
};
...
const signUp = async (req: Request, res: Response) => {
  ...
  try {
    // Token tekshiruvi
    const decoded = jwt.verify(tokenParts[1], process.env.JWT_SECRET!) as {
      phoneNumber: string;
      isPhoneVerified: boolean;
      userId: string;
    };
    ...
    const addresses = req.body.addresses.map((addr: any, index: number) => ({
      lng: addr.lng,
      lat: addr.lat,
...
const signIn = async (req: Request, res: Response) => {
  ...
  try {
    let entity;
    let isMatch = false;
    const user = await User.findOne({ phoneNumber: phone_number }).select(
      "+password"
    );
    if (!user) {
    ...
    // Restaurant owner authentication
      ...
      const restaurant = await Restaurant.findOne({ phone_number }).select(
        "+password"
      );
    ...
    const tokenPayload = {
      ...
      role,
      ...(role === "restaurant_owner" && { restaurantId: entity._id }),
    };
    ...
    if (role === "restaurant_owner") {
      const { password: _, ...restaurantData } = entity.toObject();
